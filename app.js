         ...
         //  console.log(location);
           ...
           if(data.loadFailed == false){
             ...
             console.log(data.images[0])
               ...
               messagesAdd = '<div style="width:100%; font-size:1em">' + data.title + data.description + '</div>'
               messagesAdd += '<div style="width:100%; font-size:1em"><span style="color:grey; width:100%; font-size:1em">' +  senderName + '</span>' + '<br>' + '<span>' + msg.substring(0, location) + '<a href='+ currLink + '>' + msg.substring(location, location + currLink.length+1)  + '</a>'+ msg.substring(location + currLink.length+1) + '</span></div>';
               //socket.emit('addToChat')
               socket.emit('addMessages', messagesAdd, otherUser);
...
"sort" : [
  {"date" : {"order" : "asc"}}
      ...
      for (i in hits){
        ...
        if(match.size != 0){
          ...
          var senderName = hits[i]._source.sender_username;

          //EDIT HERE FOR LINK PREVIEW
          if(senderName == userCurrent){
          messagesToAdd += '<div style="width:65%; background-color:#FFC166; font-size:1em; text-align:left; margin-left:6px; margin-right:6px; margin-bottom:6px; float:right"><span style="color:black; font-weight:bold; font-size:1em;">' + senderName + '</span>' + '<br>' + '<span style="word-wrap:break-word">' + msg.substring(0, location) + '<a href='+ currLink + '>' + msg.substring(location, location + currLink.length+1)  + '</a>'+ msg.substring(location + currLink.length+1) + '</span></div></div><br>'
          }
          else {
            messagesToAdd += '<div style="width:65%; background-color:white; font-size:1em; text-align:left; margin-left:6px; margin-right:6px; margin-bottom:6px; float:left"><span style="color:black; font-weight:bold; font-size:1em;">' + senderName + '</span>' + '<br>' + '<span style="word-wrap:break-word">' + msg.substring(0, location) + '<a href='+ currLink + '>' + msg.substring(location, location + currLink.length+1)  + '</a>'+ msg.substring(location + currLink.length+1) + '</span></div></div><br>'
          }
        ...
        if(hits[i]._source.sender_username == userCurrent){
          //#ffbf80
        messagesToAdd += '<div style="width:65%; background-color:#FFC166; font-size:1em; text-align:left; margin-left:6px; margin-right:6px; margin-bottom:6px; float:right"><span style="color:black; font-weight:bold; font-size:1em;">' + hits[i]._source.sender_name + '</span>' + '<br>' + '<span">' + hits[i]._source.message + '</span></div></div><br>'
      }
      else {
        //#80ccff
        messagesToAdd += '<div style="margin-left:6px; margin-bottom:6px; margin-right:6px; width:65%; float:left; background-color:white"><span style="color:black; font-weight:bold;width:100%; font-size:1em; display:inline">' + hits[i]._source.sender_name + '</span>' + '<br>' + '<span>' + hits[i]._source.message + '</span></div>' + '<br>'
      }
    }
      ...
      if(hits[i]._source.attachment != null){
          if(hits[i]._source.sender_username == userCurrent){
        messagesToAdd += '<div style="width:65%; background-color:#FFC166; font-size:1em; text-align:left; margin-left:6px; margin-right:6px; margin-bottom:6px; float:right; word-wrap: break-word;"><span style="color:grey; font-size:1em;">' + hits[i]._source.sender_name + '</span>' + '<br>' + '<span><a href=/upload/'+hits[i]._source.file_name + ' target="_blank">' + hits[i]._source.file_name + '</a></span></div><br>';
      }
      else {
        messagesToAdd += '<div style="width:65%; background-color:white; font-size:1em; text-align:left; margin-left:6px; margin-right:6px; margin-bottom:6px; float:left"><span style="color:grey; font-size:1em; word-wrap: break-word;">' + hits[i]._source.sender_name + '</span>' + '<br>' + '<span><a href=/upload/'+hits[i]._source.file_name + ' target="_blank">' + hits[i]._source.file_name + '</a></span></div><br>';
      }
      }
      ...
      if(recentMessage != null){
        ...
        if(getUrls(recentMessage, {'stripWWW': false}).size != 0){
        //FIX THIS!!!!!!!!
        var setIter = match.values();
        var currLink = setIter.next().value
        ...
        messagesToAdd += '<a href='+ "'" + recentMessage + "'" + '>'+ recentMessageSecond + '</a>';
        }
        else {
  ...
  "sort" : [
    {"date" : {"order" : "desc"}}
        ...
        //console.log(hits);
        messagesToAdd = '<table style="width:100%; height:100%;" cellspacing=18><tr><td style="color:grey; width:60%;">Date</td><td style="color:grey; width:20%;">User</td><td style="color:grey; width:20%;">Message</td></tr>';
        for (i in hits){
          console.log(hits[i]._source.message)

          var date = new Date(hits[i]._source.date);
          messagesToAdd += '<tr><td>' + date + '</td><td>' + hits[i]._source.sender_name + '</td><td>' + hits[i]._source.message + '</td></tr>';
